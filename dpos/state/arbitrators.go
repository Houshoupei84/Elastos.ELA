...
type arbitrators struct {
	...
	*degradation
	...
	//key is node public key string, value is whether illegal
	arbitersIllegal map[string]bool
...
func (a *arbitrators) distributeDPOSReward(height uint32,
	...
	if height >= a.chainParams.IllegalBehaviorPenaltyHeight+2*uint32(len(a.currentArbitrators)) {
	...
	for _, arbiter := range a.currentArbitrators {
		...
		if _, ok := a.currentCRCArbitersMap[ownerHash]; ok {
			...
			if !ok || m.crMember.MemberState != state.MemberElected || m.crMember.DPOSPublicKey == nil {
				...
				if m.crMember.MemberState == state.MemberIllegal || m.crMember.MemberState == state.MemberInactive {
